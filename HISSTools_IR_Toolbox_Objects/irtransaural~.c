...
void irtransaural_process_internal(t_irtransaural *x, t_symbol *sym, short argc, t_atom *argv)
{
    ...
    spectrum_1.realp = ALIGNED_MALLOC(sizeof(double) * fft_size * 5);
    ...
    else
    {
        ...
        for (i = 1; i < fft_size_halved; i++)
        {
            ...
            t1 = CSET(a, b);
            t2 = CSET(c, d);
            t3 = CSUB(CMUL(t1, t1), CMUL(t2, t2));

            spectrum_2.realp[i] = -c;
            spectrum_2.imagp[i] = -d;
            spectrum_3.realp[i] = CREAL(t3);
            spectrum_3.imagp[i] = CIMAG(t3);
        }
