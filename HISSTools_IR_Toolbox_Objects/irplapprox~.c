...
typedef struct _irpiecewiseapprox
{
    t_pxobject x_obj;
...
typedef struct _PLA_data
{
    AH_UIntPtr start_pos;
    AH_UIntPtr end_pos;
    
    struct _PLA_data *next_data;
...
void irpiecewiseapprox_process_internal(t_irpiecewiseapprox *x, t_symbol *source, short argc, t_atom *argv)
{
    ...
    spectrum_1.realp = ALIGNED_MALLOC(sizeof(double) * ((fft_size * 2) + (fft_size_halved + 2)));
    spectrum_1.imagp = spectrum_1.realp + fft_size;
    log_freqs = spectrum_1.imagp + fft_size;
    
    in = ALIGNED_MALLOC(source_length * sizeof(float));    
